...
Already included in every service's `package.json`:
...
Swagger is configured in `main.ts` — already in place for all services. It is gated by `NODE_ENV !== 'production'`:
...
```typescript
@ApiTags('Users')           // groups routes in Swagger UI
@ApiBearerAuth()            // marks routes as requiring Bearer token
@Controller('users')
