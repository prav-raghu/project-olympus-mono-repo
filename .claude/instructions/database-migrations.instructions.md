...
Only after confirming no running code references it. Deploy a version that removes all references, then drop in the next migration.

### Adding an index on a large table

MySQL's `ALTER TABLE ... ADD INDEX` can lock the table for the duration on older storage engines. On InnoDB, use `ALGORITHM=INPLACE, LOCK=NONE` explicitly via raw SQL in the migration file once a table exceeds roughly 1M rows, rather than a plain Prisma `@@index`:

```sql
ALTER TABLE products ADD INDEX idx_products_slug (slug), ALGORITHM=INPLACE, LOCK=NONE;
```
...
public async backfillSlugs(): Promise<void> {
  ...
  do {
    ...
    await this.prisma.$transaction(
      batch.map((p) => this.prisma.product.update({ where: { id: p.id }, data: { slug: generateSlug(p.name) } })),
    );
...
The deployment's rollout waits for this Job to complete before rolling out new Pods.
...
- [ ] Large table index additions avoid a full-table lock
- [ ] Column/table renames follow expand-contract, not direct rename
- [ ] `prisma migrate deploy` is run by the K8s Job before new app Pods start
