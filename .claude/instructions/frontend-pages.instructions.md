...
- **Typed inputs**: Use Angular `input()` signal for component inputs — not the `@Input()` decorator
...
@if (items().length === 0) {
  <div class="flex flex-col items-center py-12">
    ...
    <a routerLink="/items/new" class="px-4 py-2 bg-primary text-primary-foreground rounded-lg">Add First Item</a>
  </div>
}
```
