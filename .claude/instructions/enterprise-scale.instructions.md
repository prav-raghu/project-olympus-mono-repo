...
**Graceful shutdown**: NestJS handles `SIGTERM` automatically via the `OnApplicationShutdown` lifecycle hook — implement it on `DatabaseModule`/connection-holding modules to close connections cleanly.

**Health endpoints**: Every service MUST expose via `HealthModule`:

- `GET /health` (liveness) — returns 200 if the process is alive
- `GET /health/ready` (readiness) — returns 200 only if MySQL + Redis connections are healthy
...
**Query efficiency**: Use Prisma `select` on all list/search queries. Never fetch full rows on paginated endpoints.
...
**Signals**: Use `signal()`, `computed()`, `effect()` for component state — not RxJS `BehaviorSubject`.

**Debounced search**: All search inputs debounce 300ms before triggering API calls.

**Optimistic UI**: Mutations update UI immediately where it makes sense, roll back on error.
